...
def editCategorie(categorie_id):
    if 'username' not in login_session:
        return redirect('/login')
    editedCategorie = session.query(
        Categories).filter_by(id=categorie_id).one()
    if editedCategorie.user_id != login_session['user_id']:
        return "<script>function myFunction() \
                {alert('You are not authorized to edit this categorie. \
                Please create your own categorie in order to edit.');} \
...
def deleteCategorie(categorie_id):
    if 'username' not in login_session:
        return redirect('/login')
    categorieToDelete = session.query(
        Categories).filter_by(id=categorie_id).one()
    itemsCategorieToDelete = session.query(
        CategorieItem).filter_by(categorie_id=categorie_id).all()
    if categorieToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized \
               to delete this categorie. Please create your own categorie in \
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'deletecategorie.html',
            categorie=categorieToDelete, categorie_id=categorie_id)
...
def categorieItems(categorie_id):
    ...
    if 'username' not in login_session \
        ...
        return render_template(
            'publiccategorieitem.html', items=items,
            categorie=categorie, creator=creator, categorie_id=categorie_id)
    else:
        return render_template(
            'categorieitem.html', categorie=categorie, items=items,
            creator=creator, categorie_id=categorie_id)
...
def newCategorieItem(categorie_id):
    ...
    if request.method == 'POST':
        newItem = CategorieItem(
            ...
            description=request.form['description'],
            categorie_id=categorie_id, user_id=login_session['user_id'])
        session.add(newItem)
        session.commit()
        return redirect(url_for(
            'categorieItems', categorie=categorie,
            items=items, categorie_id=categorie_id))
    else:
        return render_template(
            'newitem.html', categorie_id=categorie_id,
            categorie=categorie)
...
def editCategorieItem(item_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        return redirect(url_for('categorieItems',
                                categorie_id=editedItem.categorie_id))
    else:
        return render_template(
            'edititem.html',
            categorie_id=editedItem.categorie_id,
...
def deleteCategorieItem(item_id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for(
            'categorieItems',
            categorie_id=itemToDelete.categorie_id))
    else:
        return render_template(
